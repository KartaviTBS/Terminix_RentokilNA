report 50027 "ARC NAPC BOL Order Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/Reports/NAPCBOLOrder.Report.rdl';
    Caption = 'BOL Order Report';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem("ARC NAPC BOL Header"; "ARC NAPC BOL Header")
            {
                ...
                trigger OnPreDataItem();
                begin
                    "Sales Header".CalcFields("ARC NAPC Bill of Lading No.");
                    SetRange("No.", "Sales Header"."ARC NAPC Bill of Lading No.");
                end;
    ...
    begin
        with "Sales Header" do begin
            ...
            if "ARC NAPC Bill of Lading No." = '' then begin
                ...
                NAPCBOLHeader."Source Doc. Type" := NAPCBOLHeader."Source Doc. Type"::"Sales Order";
                ...
                HazMatOnly := NAPCBOLHeader."Shipping Agent Code" in ['UPS', 'DELIVERY', 'PICKUP'];
                NAPCBOLManagement.GetSourceLines(NAPCBOLHeader."Source Doc. Type", NAPCBOLHeader."Source Doc. No.", NAPCBOLHeader);//new
                NAPCBOLManagement.BuildBOLSummaryLines(NAPCBOLHeader."No.", TempNAPCBOLSummaryLine);
                NAPCBOLManagement.SetAlternateBOLCodes(TempNAPCBOLSummaryLine, HazMatOnly);
                if TempNAPCBOLSummaryLine.FindSet() then begin
                    NAPCBOLSummaryLine.SetRange("NAPC BOL Document No.", NAPCBOLHeader."No.");
            ...
            FormatAddress.SalesOrderShipTo(ShipToAddress, "Sales Header");
            if Location.Get("Location Code") then
                FormatAddress.Location(ShipFromAddress, Location);
            if ShipToAddr.Get("Sales Header"."Sell-to Customer No.", "Sales Header"."Ship-to Code") then
